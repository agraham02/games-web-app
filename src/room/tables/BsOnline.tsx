"use client";

/**
 * BS, in a room.
 *
 * Almost nothing here is new, which is the whole return on building the
 * engine for two drivers from the start: `GameHostView` renders the table,
 * `BsTable` supplies the game-specific band and rail, and `useOnlineRuntime`
 * presents the server's frames in the shape both already consume. What this
 * file adds is only what genuinely differs online — who you are, what the
 * other people are called, and a way back to the lobby.
 *
 * The one thing worth knowing about THIS game online: the challenge window is
 * a real race over the wire. Several people are entitled at once, every one of
 * their clients is drawing its own ring, and the server decides who actually
 * got there first. That works because nothing here is authoritative: a client
 * ring only ever removes its own owner from the race, and
 * `GameDefinition.deadline` on the server is what really resolves a window —
 * which matters, because a backgrounded tab has its timers throttled to about
 * one a minute and a client clock cannot be what makes a shared table move.
 */

import { useMemo } from "react";
import type { PieceId, SeatId } from "@/engine/types";
import { botColour } from "@/games/_shared/botIdentity";
import type { BsAction, BsState } from "@/games/bs/types";
import { GAMES } from "@/session/registry";
import { GameHostView } from "@/table/GameHost";
import { Button } from "@/ui/primitives/Button";
import {
  BsTable,
  clearPlayCards,
  onPieceTap as tapCard,
  pendingLabel,
  playerViews,
  roundSummary,
  standings,
  statsFor,
  togglePlayCard,
  type BsView,
} from "@/app/play/bs/table";
import { tintFor } from "../Roster";
import { awayFrom, openingPosition, useOnlineRuntime } from "../useOnlineRuntime";
import type { OnlineTableProps } from "../tables";

export function BsOnline({ api, room, frame, held, onClearHeld, setHeld }: OnlineTableProps) {
  // Through the shared rule, not a bare toggle. `RoomScreen` owns WHERE the
  // selection lives (so it survives a trip to the lobby); what a tap means is
  // a rules question with one answer, and it lives next to `onPieceTap` in
  // the table.
  const pickUp = (id: PieceId) => setHeld((prev) => togglePlayCard(prev, id));
  const putDown = () => {
    clearPlayCards(held);
    onClearHeld();
  };

  const definition = useMemo(() => {
    const entry = GAMES[room.gameId ?? "bs"];
    return entry.create(room.settings);
  }, [room.gameId, room.settings]);

  const live = useOnlineRuntime<BsState, BsAction>({
    frame,
    submit: (action) => api.send({ t: "action", action }),
    nextRound: () => api.send({ t: "nextRound" }),
    // The undealt table, so the opening deal has a pile to fly from.
    initial: () => openingPosition(definition, room.seats, frame.seat),
  });

  const view: BsView = useMemo(
    () => ({
      // A spectator has no seat. `-1` matches no seat anywhere, which is
      // exactly the effect wanted: nothing on the table is "yours", and every
      // hand at it stays face down.
      viewerSeat: frame.seat ?? -1,
      nameFor: (seat: SeatId) => frame.seatNames[seat] ?? `Bot ${seat + 1}`,
      colourFor: (seat: SeatId) => {
        const owner = room.members.find((m) => m.seat === seat);
        return owner ? tintFor(owner.session) : botColour(seat);
      },
      // A seat somebody owns but is not currently in — see `awayFrom`.
      awayFor: awayFrom(frame),
    }),
    [frame, room.members],
  );

  // Only ever null for a moment before there is a frame at all: the table is
  // drawn from the first frame, and its deal plays once this player's own
  // table is up.
  if (!live) return <TableWaiting onLeave={api.exitGame} />;

  return (
    <div className="relative h-svh">
      <GameHostView<BsState, BsAction>
        definition={definition}
        runtime={{ seats: room.seats }}
        gameTitle={GAMES[room.gameId ?? "bs"].name}
        viewerSeat={frame.seat}
        serverDriven
        live={live}
        players={(state, l) => playerViews(view, state, l)}
        standings={(state, l, seats) => standings(view, state, l, seats)}
        stats={(state) => statsFor(view, state)}
        roundSummary={(state) => roundSummary(view, state)}
        pendingLabel={(state, seat) => pendingLabel(view, state, seat)}
        onPieceTap={(id, l) => tapCard(view, id, l, pickUp)}
        onLobby={() => {
          putDown();
          api.exitGame();
        }}
      >
        {(l) => <BsTable view={view} live={l} held={held} onClearHeld={putDown} />}
      </GameHostView>

      {/*
        The only exit from a game is back to the lobby — leaving the room
        outright is a lobby action. Deliberately a small corner control rather
        than anything in the hand band: that band has one owner (`HandZone`),
        and a second claimant on it is how a layout starts fighting itself.
      */}
      <div className="absolute top-2 right-2 z-[1900] flex gap-2">
        {room.youAreLeader ? (
          <Button size="sm" tone="danger" onClick={api.endGame}>
            End game
          </Button>
        ) : null}
        <Button size="sm" onClick={api.exitGame}>
          {frame.seat === null ? "Stop watching" : "Step away"}
        </Button>
      </div>
    </div>
  );
}

function TableWaiting({ onLeave }: { onLeave: () => void }) {
  return (
    <main className="felt felt-weave flex h-svh flex-col items-center justify-center gap-4">
      <span className="eyebrow">Dealing you in</span>
      <Button size="sm" onClick={onLeave}>
        Back to the lobby
      </Button>
    </main>
  );
}
