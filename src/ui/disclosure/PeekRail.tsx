...
/**
 * A draggable panel that lives INSIDE the table, not over the page.
 *
 ...
 *
 * A right-edge variant of this was built and then removed: a short
 * viewport has no vertical budget for a sheet, but the answer chosen for
 * that case is for the game to decline to lay out at all (Rummy asks the
 * player to rotate) rather than to grow a second layout nobody was
 * playing on. It is in this file's history if a game ever wants it.
 *
 ...
 * the same value that renders at rest and animates on settle leaves no
 * seam for the two to desync across. There is nothing to keep in sync
 * because there is only one thing.
 *
 * A second consequence of the old shape is gone with it: `y` translated
 * the box without changing its real size, and `translateY` moves where a
 * box PAINTS, not whether the browser still resolves clicks against the
 * space it vacated — so an inflated-but-translated rail silently ate
 * taps on the hand cards underneath it. A real size has no vacated
 * space.
 */
...
export interface PeekRailProps {
  /** Visible heights in px, ascending. Typically [peek, full]. */
  ...
  /**
   * Distance from the container's bottom, in px. Games pass the
   * hand-zone height so the rail stops above the player's cards — even
   * fully open it must never cover your own hand, or "check the board"
   * turns into "lose sight of what you were comparing".
   */
  offsetBottom?: number;
}
...
export function PeekRail({
  ...
  const extent = useMotionValue(snapPoints[snapIndex] ?? 0);
  ...
  // animation to the extent it was already at) on every unrelated
  // re-render of the page. That is a real source of the rail feeling
  // unsettled.
  const target = snapPoints[snapIndex] ?? 0;
  useEffect(() => {
    if (dragging.current) return;
    animate(extent, target, TRANSITIONS.ui);
  }, [target, extent]);
  ...
  // and this element is simultaneously animating its own size — so the
  // rail was being translated AND resized from one gesture, which is the
  // jank. Reading the client coordinate directly and feeding only the
  // extent keeps exactly one thing moving.
  const gesture = useRef<{ id: number; at: number; from: number; moved: boolean } | null>(null);

  const onPointerDown = (e: React.PointerEvent) => {
    (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
    dragging.current = true;
    gesture.current = { id: e.pointerId, at: e.clientY, from: extent.get(), moved: false };
  };

  const onPointerMove = (e: React.PointerEvent) => {
    const g = gesture.current;
    if (!g || g.id !== e.pointerId) return;
    const delta = g.at - e.clientY; // Dragging UP makes it taller.
    if (Math.abs(delta) > 3) g.moved = true;
    extent.set(Math.max(smallest, Math.min(tallest, g.from + delta)));
  };
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    // point, so the rail is fully usable without dragging at all.
    if (!g.moved) {
      animate(extent, snapPoints[(snapIndex + 1) % snapPoints.length] ?? 0, TRANSITIONS.ui);
      onSnapChange((snapIndex + 1) % snapPoints.length);
      return;
    }

    const current = extent.get();
    let best = 0;
    ...
    animate(extent, snapPoints[best] ?? 0, TRANSITIONS.ui);
    onSnapChange(best);
  };
  ...
  return (
    <motion.section
      ...
      style={{ height: extent, bottom: offsetBottom }}
    >
      {/* Grab handle — and the only drag surface. Confining the gesture
          here leaves the rail's own contents free to scroll normally,
          which a whole-panel drag handler fights with. */}
      <div
        role="button"
        tabIndex={0}
